...
class Test_PatchedConic(unittest.TestCase):
    ...
    def test_init_patched_conic(self):
        ...
        x0 = init_patched_conic((leo.r, leo.v + 3140.0,       0.0, 0.0, 1937000.0))
        x1 = init_patched_conic(x0, np.array([[dlam1], [dv0]]))
    ...
    def test_dv0_gradients(self):
        ...
        leo = Orbit.circular(PatchedConic.mu_earth, 6371400.0 + 185000.0)
        x0 = init_patched_conic((leo.r, leo.v + 3140.0,       0.0, 0.0, 1937000.0))
        x1 = init_patched_conic((leo.r, leo.v + 3140.0 + dv0, 0.0, 0.0, 1937000.0))
        ...
        drpl = x0.drpl_dv0 * dv0
        drpl_ = x1.rpl - x0.rpl

        dvpl = x0.dvpl_dv0 * dv0
        dvpl_ = x1.vpl - x0.vpl

        dphi1 = x0.dphi1_dv0 * dv0
        dphi1_ = x1.arrive.phi - x0.arrive.phi

        dphi2 = x0.dphi2_dv0 * dv0
        ...
        np.testing.assert_approx_equal(dvpl, dvpl_, significant=3)
        np.testing.assert_approx_equal(drpl, drpl_, significant=3)
        ...
        np.testing.assert_approx_equal(dg, dg_, significant=3)
        np.testing.assert_approx_equal(df, df_, significant=3)
